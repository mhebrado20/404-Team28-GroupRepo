...
void phoneState1() {
  ...
  if (digitalRead(RING_PIN) == HIGH) {
        ...
        if () { // cid matches blacklist
          ...
          if ((ringTimeCounter > 36) && ((ONOFFHOOK_PIN != HIGH) || (ONOFFHOOK_WHYS_PIN != HIGH))) {
            timerEnd(ringTimer);
            ringTimer = NULL;
...
void phoneState2() {
  ...
  if () { // CPE Alerting Signal (CAS) detected
    // pause first call
    // go into second call, get cwid
    // start call 2 recording and timer
  ...
  if (digitalRead(ONOFFHOOK_PIN) == LOW) {
    phoneState = ONHOOK_CALLEND;
  }
